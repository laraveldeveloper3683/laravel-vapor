    ...
    for ($i=0; $i < $request->counter; $i++) {
      ...
      foreach ($request->json_data['observations'] as $key => $value) {
        ...
        foreach ($last_wells_table_data as $data) {
          ...
          $observation->azure_classification = (string) isset($value['azure_classification']) ? $value['azure_classification'] : null;
          $observation->azure_ct = (string) isset($value['azure_ct']) ? $value['azure_ct'] : null;
          $observation->classification = (string) isset($value['classification']) ? $value['classification'] : null ;
